<template>
  <div id="taobaoAuth">
    <div class="taobaoAuth">
      <div class="titleImg"><img src="../../../assets/img/taobao.png" alt=""></div>
      <mt-field placeholder="请输入本人淘宝账号" v-model="formData.phone"  :attr="{maxlength:11}"></mt-field>
...
export default {
  ...
  methods: {
    submitData(){
      ...
      if (success){
        this.loading = true
        this.httpRequest.taoBaoPreLogin({
          mobile:this.formData.phone,
          password:this.formData.password,
        }).then((res)=>{
          console.log('淘宝登录',res)
          this.loading = false
          if(res.code == '00000000'){

            if(res.data.code == '000000' || res.data.code == '000009'){
              this.taobaoMsgValidate(this.formData.phone,res.data.token)
            }else if(res.data.code == '000001'){
              this.token=res.data.token
              sessionStorage.setItem('taobao',this.formData.phone)
              this.$router.push({path:'/taobaoCode',query:{token:res.data.token}})
            }else{
  ...
  activated(){
    localStorage.setItem('headerTitle','淘宝认证')
  }
}
</script>
